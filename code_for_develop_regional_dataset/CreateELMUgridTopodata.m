function CreateELMUgridTopodata(lati_region, long_region, ...
    clm_gridded_topodata_filename, ...
    out_netcdf_dir, clm_usrdat_name,is_0_360)

if is_0_360
    long_region(long_region>180) = long_region(long_region>180) - 360;
end
fname_out = sprintf('%s/topodata_%s_%s.nc',out_netcdf_dir,clm_usrdat_name,datestr(now, 'cyymmdd'));
disp(['  Topo_dataset: ' fname_out])
...
ncid_inp = netcdf.open(clm_gridded_topodata_filename,'NC_NOWRITE');
ncid_out = netcdf.create(fname_out,'NC_CLOBBER');

info_inp = ncinfo(clm_gridded_topodata_filename);
...
% pftmask = ncread(clm_gridded_topodata_filename,'PFTDATA_MASK');
...
for ii=1:size(long_region,1)
    for jj=1:size(long_region,2)
        ...
        dlong = abs(longxy - long_region(ii,jj));
        dlong(dlong>180) = 360 -  dlong(dlong>180);
        %%%%%%%%%%%%%%%%%%%
        
        dist = dlong.^2 + (latixy - lati_region(ii,jj)).^2;
