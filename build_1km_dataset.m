...
Silts = nan([size(silt_0_5), 6]);
Silts(:,:,1) = silt_0_5;
Silts(:,:,2) = silt_5_15;
Silts(:,:,3) = silt_15_30;
Silts(:,:,4) = silt_30_60;
Silts(:,:,5) = silt_60_100;
Silts(:,:,6) = silt_100_200;
...
for row_i = 1:rows
    ...
    for  col_i = 1:cols
        ...
        elseif LC_i == 17 %urban
            PCT_URBAN(row_i,col_i, 1) = 100; % TBD type
            PCT_NAT_PFT( row_i,col_i, 1) = 100;
        elseif LC_i == 18 %lake
        ...
        silt_old = squeeze(Silts(row_i,col_i,:));
        dom_old = squeeze(Doms(row_i,col_i,:));
        
        clay_new = interp1(z_old, clay_old, z_new,'linear', 'extrap');
        sand_new = interp1(z_old, sand_old, z_new,'linear', 'extrap');
        silt_new = interp1(z_old, silt_old, z_new,'linear', 'extrap');
        clay_new(clay_new<0) = 0;
        sand_new(sand_new<0) = 0;
        silt_new(silt_new<0) = 0;
...
save('variables_ELM.mat','MONTHLY_LAI','MONTHLY_SAI',...
    ...
    'PCT_LAKE','PCT_NATVEG','PCT_URBAN','PCT_WETLAND','PCT_NAT_PFT','PCT_GLACIER',...
    'TOPO','SLOPE','STD_ELEV','-v7.3');
